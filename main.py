...
class TimerMenu(Vertical):
    ...
    def on_input_key(self, event: events.Key) -> None:
        """Move focus back to the options when pressing Up."""
        if event.key == "up":
            option_list = self.query_one(TimerOptionList)
...
class NotificationBar(Static):
    ...
    def show(self, message: str, duration: float = 2.0) -> None:
        """Display a message and automatically fade it out."""
        ...
        def fade_out() -> None:
            # Fade the bar away and slide it down again.  ``styles.animate`` is
            # used for CSS properties like opacity because the ``animate``
            # method only works with widget attributes.  The bar is moved back
            # off screen at the same time so it is ready for the next message.
            # Fade out over half a second which is twice as long as the fade in
            self.styles.animate("opacity", 0.0, duration=0.5, on_complete=self._hide)
    ...
    def save_tab_state(self) -> None:
        ...
        data = {
            ...
            "tabs": [
                {
                    "id": tab_id,
                    "title": self.tabs.get_pane(tab_id).title,
